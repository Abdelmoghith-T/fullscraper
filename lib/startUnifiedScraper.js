...
async function startUnifiedScraper({
  ...
  // ✅ ENHANCED: Validate user API keys - NO FALLBACKS ALLOWED - NOW REQUIRES 3 KEYS
  if (!apiKeys.googleSearchKeys || !Array.isArray(apiKeys.googleSearchKeys) || apiKeys.googleSearchKeys.length !== 3) {
    throw new Error('User must provide exactly 3 valid Google Search API keys. No fallback keys allowed.');
  }

  if (!apiKeys.geminiKeys || !Array.isArray(apiKeys.geminiKeys) || apiKeys.geminiKeys.length !== 3) {
    throw new Error('User must provide exactly 3 valid Gemini API keys. No fallback keys allowed.');
  }
  ...
  if (validGoogleKeys.length !== 3) {
    throw new Error(`User must provide exactly 3 valid Google Search API keys. Found ${validGoogleKeys.length} valid keys out of 3 provided.`);
  }

  if (validGeminiKeys.length !== 3) {
    throw new Error(`User must provide exactly 3 valid Gemini API keys. Found ${validGeminiKeys.length} valid keys out of 3 provided.`);
  }
