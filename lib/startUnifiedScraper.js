...
async function startUnifiedScraper({
  ...
  // ✅ ENHANCED: Validate user API keys based on user type (trial vs paid)
  const isTrialUser = apiKeys.googleSearchKeys.length === 1 && apiKeys.geminiKeys.length === 1;
  const isPaidUser = apiKeys.googleSearchKeys.length === 3 && apiKeys.geminiKeys.length === 3;

  if (!isTrialUser && !isPaidUser) {
    throw new Error('Invalid API key configuration. Trial users need 1 key each, paid users need 3 keys each.');
  }
  ...
  if (validGoogleKeys.length !== apiKeys.googleSearchKeys.length) {
    throw new Error(`User must provide ${apiKeys.googleSearchKeys.length} valid Google Search API keys. Found ${validGoogleKeys.length} valid keys out of ${apiKeys.googleSearchKeys.length} provided.`);
  }

  if (validGeminiKeys.length !== apiKeys.geminiKeys.length) {
    throw new Error(`User must provide ${apiKeys.geminiKeys.length} valid Gemini API keys. Found ${validGeminiKeys.length} valid keys out of ${apiKeys.geminiKeys.length} provided.`);
  }

  const userType = isTrialUser ? 'Trial' : 'Paid';
  console.log(`🔑 ${userType} User API Keys Validated:`);
  console.log(`   📍 Google Search: ${validGoogleKeys.length} valid key${validGoogleKeys.length > 1 ? 's' : ''}`);
  console.log(`   🤖 Gemini AI: ${validGeminiKeys.length} valid key${validGeminiKeys.length > 1 ? 's' : ''}`);
  ...
  const {
    ...
    onBatch = () => {},
    onProgress = () => {},
    // Unlimited by default; capped separately for free_trial
  ...
  // Validate maxResults only if finite and positive
  ...
  try {
    ...
    // Initialize source manager with error handler
    const sourceManager = new SourceManager(errorHandler);
    ...
    sourceManager.processAndSaveResults = async function(results, source, dataType, niche, format) {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        // Stop if we've reached max results (only if finite)
        if (Number.isFinite(maxResults) && processedCount >= maxResults) {
      ...
      // Trim results to max limit if finite
      if (Number.isFinite(maxResults) && allResults.length > maxResults) {
    ...
    const results = await sourceManager.run(niche, scraperSource, dataType, scraperFormat, {
      ...
      trialMode,
      trialLimit,
      abortSignal // Pass the abort signal to the source manager
    });
    ...
    // If in trial mode, deduplicate and cap to trialLimit BEFORE export
    if (trialMode) {
      const deduped = deduplicateGeneric(allResults);
      allResults = deduped.slice(0, Math.max(0, trialLimit || 20));
      console.log(`🔒 Trial mode: results deduped and limited to ${allResults.length}`);
    }
    ...
    const cleanNiche = niche.replace(/[<>:"/\\|?*]/g, '_').replace(/\s+/g, '_').toLowerCase();
    ...
    // BUT in trial mode we always export a trimmed file to enforce the 20-result cap
    if (source === 'LINKEDIN' && !trialMode) {
    ...
    const meta = {
      ...
      maxResultsLimit: trialMode ? (trialLimit || 20) : (Number.isFinite(maxResults) ? maxResults : null),
      trialMode: !!trialMode,
      phase: 'completed'
    };
...
// Generic deduplication suitable across sources
function deduplicateGeneric(results) {
