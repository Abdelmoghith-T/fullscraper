...
class AdminManager {
  ...
  modifyUserApiKeys(adminCode, userCode, newApiKeys) {
    ...
    if (newApiKeys.googleSearchKeys.length === 0 || newApiKeys.geminiKeys.length === 0) {
      return { success: false, error: 'At least one Google Search key and one Gemini key are required.' };
    }
